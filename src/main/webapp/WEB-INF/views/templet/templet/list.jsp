...
<div class="navbar">
    <div class="navbar-inner no-bg">
        <h4 class="title"><i class="fontello-icon-window"></i>模板管理 
        	<small>        	
			...
			<c:if test="${empty parent}">
				<a href="${appPath}/templet/templet/add.do" class="btn btn-blue">添加</a>
			</c:if>		
			</small>
		</h4>
    </div>		
    ...
    <tbody>
       <c:forEach items="${resLst.content}" var="obj" varStatus="status">
            <tr>
                ...
                <td>${obj.level}</td>
                <td>${obj.remark}</td>
                <td><a href="${appPath}/templet/templet/edit.do?id=${obj.id}">修改<a>
                	<a href="${appPath}/templet/templet/del.do?id=${obj.id}">删除<a>
                </td>
            </tr>
        </c:forEach>
    </tbody>
