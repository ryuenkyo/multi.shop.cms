 ...
 <div class="padding-left20">
    ...
    <ul id="imglist" class="thumbnails thumbnails-inline">
    	<c:forEach items="${resLst.content}" var="obj" varStatus="status">
        	<li class="thumbnail square75 "><a class="nailthumb-container"><img src="${nginxUrl}${obj.httpUrl}" ></a></li>
       	</c:forEach>
    </ul>
    ...
    <tbody>
       <c:forEach items="${resLst.content}" var="obj" varStatus="status">
            <tr>
            	...
            	<td>${obj.httpUrl}</td>
                <td style="width:75px;height:75px;"><a href="${nginxUrl}${obj.httpUrl}" rel="prettyPhoto">
                	<img src="${nginxUrl}${obj.httpUrl}"><a></td>
                 <td>${obj.imageType.title}</td>
                <td>${form.imageType}/${form.fkId}
                	<c:if test="${!empty form.imageType}">
                	...
                	<c:if test="${empty form.imageType}">
                		<a href="${appPath}/biz/image_file/del.do?id=${obj.id}">删除<a>
                	</c:if>
                </td>
            </tr>
        </c:forEach>
